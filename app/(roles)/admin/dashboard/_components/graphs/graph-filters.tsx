...
const GraphFilters = ({
  ...
  const years = useMemo(() => {
    const currentYear = new Date().getFullYear(); // current year [web:62]
    const range = 6; // current year + previous 5 years
    return Array.from({ length: range }, (_, i) => String(currentYear - i));
  }, []);
  ...
  return (
    <div className="flex sm:flex-row flex-col gap-4">
      ...
      <Button className="bg-primary-bg sm:mt-0 mt-5 cursor-pointer" onClick={resetFilters}>
        Reset
      </Button>
    </div>
  );

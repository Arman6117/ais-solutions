...
type InstructorBatchCardProps = {
  batch: CourseBatch;
  courseId: string;
};
const InstructorBatchCard = ({ batch, courseId }: InstructorBatchCardProps) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-5 mb-4 hover:border-violet-300 hover:scale-102 hover:shadow-lg hover:shadow-violet-100 duration-200">
      ...
      <Button className="w-full cursor-pointer bg-primary-bg  " asChild>
        <Link
          href={`/instructor/all-batches/${courseId}/${batch._id}/batch-details?mode=view`}
        >
          View Details
...
export default InstructorBatchCard;
